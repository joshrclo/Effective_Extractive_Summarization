...
def get_basic_grad_fn(net, clip_grad, max_grad=10):
    def f():
...
@curry
def compute_loss(net, criterion, decoder, fw_args, loss_args):
    loss = criterion(*((net(*fw_args),) + loss_args + (decoder, )))
    return loss
...
@curry
def basic_validate(net, criterion, decoder, val_batches):
    print('running validation ... ', end='')
    net.eval()
    net._isTrain = False # for DeepLSTM
    start = time()
    with torch.no_grad():
        validate_fn = val_step(compute_loss(net, criterion, decoder))
...
class BasicPipeline(object):
    ...
    def get_loss_args(self, net_out, bw_args):
        if isinstance(net_out, tuple):
            loss_args = net_out + bw_args + (self._decoder, )
        else:
            loss_args = (net_out, ) + bw_args + (self._decoder, )
        return loss_args
...
class BasicTrainer(object):
    ...
    def check_stop(self, val_metric):
        if self._best_val is None:
            ...
            self._current_p += 1
        print('Current minimum loss = {}'.format(self._best_val))
